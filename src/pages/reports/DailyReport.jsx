...
// «1 сентября», «5 сентября» — день без года
const fmtDay = (ds) => {
  if (!ds) return '—';
  ...
  return parseInt(d, 10) + ' ' + (months[mi] || m) + (String(y) !== String(new Date().getFullYear()) ? ' ' + y : '');
};
...
export default function DailyReport() {
  ...
  const load = async () => {
    ...
    try {
      ...
      // Группируем по дате чека. refund_items привязаны к item чека.
      const day = {}; // dateKey -> агрегаты
      const ensure = (dk) => {
        if (!day[dk]) day[dk] = {
          dk, salesSum: 0, retSum: 0, salesCost: 0, retCost: 0, salesQty: 0, retQty: 0, alias: dk,
        };
        return day[dk];
      };

      // карта: itemId -> { index, costPerUnit }
      // себестоимость возмещения считаем по себестоимости проданной единицы того же товара
      const byId = {};
      ...
      // Продажи (net по возвратам): стоимость продажи позиции (себестоимость закупки на ед.), без услуг
      itList.forEach(it => {
        const dk = String(recById[it.receipt_id] ? recById[it.receipt_id].date || '' : '').split('T')[0];
        if (!dk) return;
        const d = ensure(dk);
        ...
        d.alias = dk;
        // продано (не смотрим возврат на этом шаге — вычтем ниже по refund)
        d.salesQty += qtySales;
        d.salesSum += totalSales;
        d.salesCost += unitCost * qtySales;
      });

      // Возвраты: refund_items лежат на чеке (r.refund_items). item_id => исходная единица чека.
      // Находим себестоимость единицы чека по product_id строки чека.
      const rowById = {};
      itList.forEach(it => { rowById[String(it.id)] = it; });
      rlist.forEach(r => {
        const dk = String(r.date || '').split('T')[0];
        if (!dk) return;
        const d = ensure(dk);
      ...
      // Чистая выручка дня = продажи − возвраты
      const list = Object.values(day).map(d => ({
        dk: d.dk,
        label: fmtDay(d.dk),
        ...
        // Прибыль = выручка(нетто) − себестоимость(нетто)
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="sub">Продажи, возвраты и прибыль по каждому дню за период</div>
        </div>
      </div>
