  ...
  return (
    <>
      <div className="sk-bar">
        ...
        <div className="***">
          <button type="button" className="sk-dd-btn" onClick={() => navigate('/stock/supply/new')}>Добавить</button>
        </div>
      </div>
      ...
      <div style={{display:'flex',alignItems:'center',gap:'4px',marginBottom:'.5rem',width:'100%',flexWrap:'nowrap',border:'1px solid '+(supplySearchFocus?'#111':'#e2e2e6'),borderRadius:'999px',padding:'5px 6px 5px 14px',background:'#fff',boxShadow:supplySearchFocus?'0 2px 10px rgba(0,0,0,.12)':'0 1px 3px rgba(0,0,0,.05)',transition:'border-color .15s, box-shadow .15s'}}
        ...
        <div className="sk-period-wrap" style={{position:'relative',display:'inline-flex',alignItems:'center',flexShrink:0}}>
          ...
          {periodOpen && (
            <div onClick={e => e.stopPropagation()} style={{display:'flex',flexDirection:'column',gap:'4px',position:'absolute',top:'100%',right:0,marginTop:'4px',background:'#fff',border:'1px solid rgba(29,120,252,.18)',borderRadius:'.85rem',boxShadow:'0 16px 40px -14px rgba(11,18,32,.3)',minWidth:'210px',padding:'.4rem',zIndex:100}}>
      ...
      <div className="sk-tablewrap" style={{flex:'none',minHeight:'auto'}}>
        ...
        <table className="sk-table sup-table">
          <thead id="supplyColHeaders">
            <tr>
              <th style={{width:'80px',minWidth:'80px',textAlign:'left',paddingRight:'4px',whiteSpace:'nowrap'}}>№</th>
            ...
            {supplies.length === 0 ? (
              ...
              return (
                ...
                <tr key={s.id} onClick={function(e){if(!e.target.closest('span')&&!e.target.closest('.prod-more-wrap'))setExpandedId(s.id === expandedId ? null : s.id)}} style={{cursor:'pointer'}}>
                  <td style={{width:'80px',minWidth:'80px',textAlign:'left',color:'#222',fontSize:'.78rem',paddingRight:'4px',whiteSpace:'nowrap'}}>{i + 1}</td>
