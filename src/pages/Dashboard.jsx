    ...
    (async () => {
      try {
        ...
        for (let dd = 1; dd <= daysInMonth; dd++) {
          ...
          const v = sumR(f, f);
          bars.push({ label: dd + '.' + String(mo + 1).padStart(2, '0'), tip: dd + ' ' + MONTHS_SHORT[mo] + ' · ' + v.toLocaleString('ru-RU') + ' ' + cur, val: v });
        }
  ...
  return (
    <div className="dash">
      ...
      <div className="card">
        ...
        <div className="bar-x">
          {(d.bars || []).filter((b, i) => i % 2 === 0).map((b, i) => <span key={i}>{b.label}</span>)}
        </div>
      </div>
