...
export default function QuickExpense({ onClose }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    supabase.from('categories').select('*').eq('user_id', user.id).eq('type', 'expense').order('name').then(({ data }) => { if (data) setCats(data); });
  }, [user]);
  ...
  const process = async () => {
    ...
    const { error } = await supabase.from('transactions').insert({
      user_id: user.id, type: 'expense', amount: parseFloat(amount),
      description: desc.trim() || catName || 'Расход',
    ...
    <div className="modal-overlay active" onClick={e => { if (e.target.className === 'modal-overlay active') onClose(); }}>
      ...
      <div className="modal-box" style={{maxWidth:'420px',padding:0}}>
        <div style={{padding:'20px 24px 0',display:'flex',justifyContent:'space-between',alignItems:'center',paddingBottom:'12px',borderBottom:'1px solid #eee'}}>
          <h2 style={{fontSize:'16px',fontWeight:700,margin:0}}>Расход</h2>
        ...
        <div style={{padding:'14px 24px 0'}}>
          ...
          <div className="form-group">
            ...
            <input type="text" value={desc} onChange={e => setDesc(e.target.value)} placeholder="Например: аренда за сентябрь" />
          </div>
        </div>
