...
export default function CategoryReport() {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const prRes = await supabase.from('products').select('id,name,cat,type,combo_items').eq('user_id', user.id);
      ...
      // Агрегируем по категории (товар/услуга/комбо — из карточки позиции)
      const agg = {};
      const ensure = (catRaw) => {
        const c = catName(catRaw);
        if (!agg[c]) agg[c] = { c, qty: 0, sum: 0, cost: 0 };
        return agg[c];
      };
      ...
      itList.forEach(it => {
        ...
        const row = ensure(pr.cat != null && pr.cat !== '' ? pr.cat : null);
        row.qty += qtyNet;
        row.sum += totalNet;
        // Себестоимость по средней закупке позиции (для комбо себестоимости в поставках нет — игнорируем его закуп как есть)
        const unitCost = avgCost[String(pr.id)] || 0;
      ...
      // «Без категории» ставим последней, если есть
      const noCat = list.find(x => x.c === 'Без категории');
      const main = list.filter(x => x.c !== 'Без категории');

      const T = list.reduce((s, x) => ({ qty: s.qty + x.qty, sum: s.sum + x.sum, cost: s.cost + x.cost }), { qty: 0, sum: 0, cost: 0 });
      setRows(noCat ? [...main, noCat] : list);
      setTotals(T);
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="sub">Продажи товаров, услуг и комбо по всем категориям за период</div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="product-table" style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
          <table className="data-table">
            <thead id="colHeaders">
              <tr>
                <th style={{ textAlign: 'left', paddingLeft: 0 }}>Категория</th>
