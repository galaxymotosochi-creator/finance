...
// Отметка «начальный остаток введен» (даже если это 0) — чтобы счет с нулем не просил ввод снова
...
export default function Accounts() {
  ...
  // Онбординг: показываем «Первоначальные остатки», пока есть невведенные счета и нет учтенных данных
  useEffect(() => {
    if (initDone && !loading) {
  ...
  // Внутренние перемещения (перевод/инкассация между своими счетами) — двигают баланс,
  // но НЕ считаются поступлениями/расходами (одни и те же деньги, чтобы не считать дважды).
  // Свои деньги владельца (взнос/вывод) — НАОБОРОТ входят: это реальные новые поступления/выводы.
  var isInternalTx = (t) => {
    if (!t) return false;
    if (t.kind === 'transfer' || t.kind === 'collection') return true;
    var d = t.description || '';
  ...
  // Счет, которому еще не ввели начальный остаток (баланс 0 и нет отметки о вводе)
  ...
  var saveInit = async (e) => {
    ...
    try {
      var anyQueued = false;
      // Считаем введенным только то, что реально заполнено: «0» — это введенный ноль,
      // пустое поле — предприниматель ничего не вводил (счет остается невведенным)
    ...
    <div style={{display:'flex',flexDirection:'column',height:'100%',minHeight:0}}>
      ...
      <div className="sk-bar">
        <div className="grow">
          <div style={{display:'flex',alignItems:'center'}}>
            ...
            <SectionHelp
              title="Раздел «Счета»"
              intro="Здесь собраны все деньги бизнеса: наличные, кассы, банковские счета и карты."
              faq={[
                { q: 'С чего начать работу? (по шагам)', a: (
                  <ol style={{paddingLeft:'1.15rem',margin:0}}>
                    <li style={{marginBottom:'.5rem'}}>Нажмите кнопку <b>«Действия ▾»</b> справа вверху — откроется меню со всеми операциями.</li>
                    <li style={{marginBottom:'.5rem'}}>Выберите <b>«Добавить счет»</b> и заведите свои счета: кассу, карту, банковский счет или сейф. Укажите название и тип.</li>
                    <li style={{marginBottom:'.5rem'}}>Снова <b>«Действия ▾»</b> → <b>«Начальные остатки»</b> — впишите, сколько денег уже лежит на каждом счете <i>до</i> начала учета. Если счет пустой, оставьте 0.</li>
                    <li style={{marginBottom:'.5rem'}}>Дальше деньги на счетах будут меняться сами — от ваших продаж и расходов в «Кассах» и «Доходах и расходах».</li>
                    <li>Нужно перекинуть деньги между счетами — <b>«Действия ▾»</b> → <b>«Перевод между счетами»</b>. Забрать наличные из кассы — <b>«Инкассация»</b>.</li>
                  </ol>
                ) },
                { q: 'Что означает каждая кнопка на странице?', a: (
                  <ul>
                    <li><b>Действия ▾</b> — меню со всеми операциями по счетам.</li>
                    <li><b>Круг сверху</b> — общий баланс всех счетов и доля каждого в процентах.</li>
                    <li><b>Строка счета</b> (клик) — история операций по этому счету.</li>
                    <li><b>⋯ в таблице</b> — редактировать или удалить счет.</li>
                    <li><b>?</b> — эта справка.</li>
                  </ul>
                ) },
                { q: 'Что делают пункты меню «Действия ▾»?', a: (
                  <ul>
                    <li><b>Добавить счет</b> — завести новый кошелек, кассовый ящик, карту или банковский счет.</li>
                    <li><b>Начальные остатки</b> — деньги, которые уже были у бизнеса до начала учета. В прибыль не попадают.</li>
                    <li><b>Корректировка</b> — исправить остаток на счете. Баланс не может уйти в минус.</li>
                    <li><b>Инкассация</b> — изъять наличные из кассового ящика и зачислить на другой счет.</li>
                    <li><b>Перевод между счетами</b> — переместить деньги со счета на счет. Это не доход и не расход бизнеса.</li>
                  </ul>
                ) },
                { q: 'Что такое «Начальные остатки» и почему их нет в прибыли?', a: (
                  <p>Это стартовая точка — сколько денег уже было на счетах до начала учета. В прибыль они не входят, иначе она была бы завышена. Изменить остаток потом можно через <b>Корректировку</b>.</p>
                ) },
                { q: 'Чем инкассация отличается от перевода?', a: (
                  <p><b>Инкассация</b> — это изъятие наличных именно из кассового ящика и зачисление на другой счет. <b>Перевод</b> — перемещение денег между любыми вашими счетами. Оба движения не считаются доходом или расходом бизнеса.</p>
                ) },
              ]}
            />
          </div>
        ...
        <>
            ...
            <table className="sk-table">
              ...
              <tbody id="dirTableBody">
                {sorted.length === 0 ? (
                  ...
                  var hasOwner = (mv.od||0) !== 0 || (mv.ow||0) !== 0 || (mv.pr||0) !== 0;
                  return (
                    <tr key={a.id}>
                ...
                {sorted.length > 0 && (() => {
                  const incTot = accounts.reduce((s,a) => { const mv=getMv(a); return s + mv.i + (mv.od||0); }, 0);
                  const expTot = accounts.reduce((s,a) => { const mv=getMv(a); return s + mv.e + (mv.ow||0) + (mv.pr||0); }, 0);
                  return (
          ...
          <div className="sk-card" style={{overflowY:'auto',overflowX:'auto',WebkitOverflowScrolling:'touch',minHeight:0}} ref={ownerTblRef} onScroll={onOwnerTblScroll}>
            <table className="sk-table">
              <thead>
                <tr>
                  <th style={{textAlign:'left'}}>Движение владельца</th>
                  <th style={{textAlign:'left'}}>Внесено / Начислено</th>
                  <th style={{textAlign:'left'}}>Выведено / Забрано</th>
                  <th style={{textAlign:'left'}}>Баланс</th>
                  <th className="actions" style={{textAlign:'left'}}></th>
                </tr>
              </thead>
              <tbody>
                <tr className="acct-sys-row" onClick={()=>setOpenOwner(!openOwner)}>
                  <td style={{textAlign:'left'}}>
                    <div style={{display:'flex',alignItems:'center',gap:'.35rem'}}>
                      ...
                      <div>
                        <div className="sk-name">Собственные средства</div>
                ...
                {openOwner && (
                  <tr className="acct-sys-det"><td colSpan={5} style={{padding:0}}>
      ...
      <Modal open={showCorrect} onClose={()=>setShowCorrect(false)} title="Корректировка баланса" subtitle="Исправьте остаток на счете" width="medium">
            <form onSubmit={async (e)=>{e.preventDefault();if(!corAmt||parseFloat(corAmt)<=0)return;var amt=parseFloat(corAmt);try{var ac=accounts.find(a=>a.id===corAcct);if(!ac)return;// Защита: баланс не может уйти в минус
              ...
              <div className="form-group">
                <label>Счет</label>
                <AcctPick accounts={accounts} value={corAcct} onChange={setCorAcct} cur={cur} balOf={balOfId} />
              </div>
      ...
      <Modal open={showInit} onClose={()=>setShowInit(false)} title={sorted.filter(notInit).length ? "Введите первоначальные остатки" : "Начальные остатки"} subtitle={sorted.filter(notInit).length ? "Введите балансы счетов. Если на счету ноль — оставьте 0 и нажмите «Сохранить»: это тоже нормально" : "Все начальные остатки уже внесены. Изменить баланс счета можно через «Корректировку»"} width="medium">
            <form onSubmit={saveInit}>
              ...
              <div className="form-group" style={{marginTop:'.75rem',paddingTop:'.75rem',borderTop:'1px solid var(--border)'}}>
                ...
                {newAccs.length === 0 && (
                  <div style={{fontSize:'.78rem',color:'var(--muted)'}}>Нажмите «+ Добавить счет», чтобы завести еще один. Все счета сохранятся кнопкой «Сохранить».</div>
                )}
              </div>
              )}
              <div className="modal-actions">
      ...
      <Modal open={showTransfer} onClose={()=>setShowTransfer(false)} title="Перевод между счетами" subtitle="Перемещение средств между счетами" width="medium">
            <form onSubmit={async (e)=>{e.preventDefault();if(!trFrom||!trTo||trFrom===trTo||!trAmt||parseFloat(trAmt)<=0)return;var amt=parseFloat(trAmt);try{var fromAc=accounts.find(a=>a.id===trFrom);var toAc=accounts.find(a=>a.id===trTo);if(!fromAc||!toAc)return;var fromBal=getBal(fromAc);if(amt>fromBal)return alert('Недостаточно средств на счете «'+fromAc.name+'». Баланс: '+fromBal.toLocaleString()+' ₽');var tid=Date.now();await supabase.from('transactions').insert({user_id:user.id,account_id:fromAc.id,type:'expense',amount:amt,description:'Перевод со счета '+fromAc.name,date:new Date().toISOString().split('T')[0],kind:'transfer',transfer_id:tid});await supabase.from('transactions').insert({user_id:user.id,account_id:toAc.id,type:'income',amount:amt,description:'Перевод на счет '+toAc.name,date:new Date().toISOString().split('T')[0],kind:'transfer',transfer_id:tid});setShowTransfer(false);await fetchTx();}catch(err){alert(err.message);}}}>
              <div className="form-group">
                <label>Откуда</label>
                <AcctPick accounts={accounts.filter(a=>String(a.id)!==String(trTo))} value={trFrom} onChange={setTrFrom} cur={cur} balOf={balOfId} placeholder="— выберите —" />
              </div>
              <div className="form-group">
                <label>Куда</label>
                <AcctPick accounts={accounts.filter(a=>String(a.id)!==String(trFrom))} value={trTo} onChange={setTrTo} cur={cur} balOf={balOfId} placeholder="— выберите —" />
              </div>
              <div className="form-group">
                <label>Сумма </label>
