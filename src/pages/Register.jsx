...
export default function Register() {
  ...
  const nextStep = () => {
    ...
    if (step === 4 && !clientType) { setError('Выберите тип клиентов'); return; }
    setError('');
    setFade('out');
    ...
    <div style={container}>
      <div style={card}>
        ...
        <h1 style={{fontSize:'1.15rem',fontWeight:700,textAlign:'center',marginBottom:2,letterSpacing:'-.03em'}}>
          {step === 1 ? 'Создать аккаунт' : step === 2 ? 'Объём продаж' : step === 3 ? 'Товары и услуги' : step === 4 ? 'Ваши клиенты' : 'Регистрация'}
        </h1>
        <p style={{fontSize:'.8rem',color:'rgba(0,0,0,.54)',textAlign:'center',marginBottom:'1rem'}}>
          {step === 1 ? 'Расскажите о своём бизнесе' : step === 2 ? 'Сколько продаёте в месяц?' : step === 3 ? 'Сколько товаров/услуг?' : step === 4 ? 'Кто ваши клиенты?' : 'Данные для входа'}
        </p>

        <Progress current={step - 1} total={5} />
