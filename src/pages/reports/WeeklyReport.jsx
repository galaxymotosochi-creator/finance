...
// «1 сентября – 7 сентября», «8 сентября – 14 сентября» — диапазон недели без года (для прошлых лет добавляется год)
const monthsShort = ['января', 'февраля', 'марта', 'апреля', 'мая', 'июня', 'июля', 'августа', 'сентября', 'октября', 'ноября', 'декабря'];
const fmtNum = (d) => parseInt(d, 10);
const fmtDayNum = (ds) => {
  if (!ds) return '';
  ...
  return fmtNum(d) + ' ' + (monthsShort[mi] || m) + (String(y) !== String(new Date().getFullYear()) ? ' ' + y : '');
};

// Ключ недели: дата понедельника недели (ISO, понедельник)
const isoDate = (ds) => {
  const p = String(ds).split('T')[0].split('-');
  const dt = new Date(Date.UTC(parseInt(p[0], 10), parseInt(p[1], 10) - 1, parseInt(p[2], 10)));
  return dt.toISOString().slice(0, 10);
};
const weekStartKey = (ds) => {
  const p = String(ds).split('T')[0].split('-');
  const y = parseInt(p[0], 10), m = parseInt(p[1], 10) - 1, d = parseInt(p[2], 10);
  const dt = new Date(Date.UTC(y, m, d));
  const dow = (dt.getUTCDay() + 6) % 7; // 0=пн..6=вс
  dt.setUTCDate(dt.getUTCDate() - dow);
  return dt.toISOString().slice(0, 10);
};
// Диапазон недели «пн – вс»
const weekRange = (monday) => {
  const p = monday.split('-');
  const dt = new Date(Date.UTC(parseInt(p[0], 10), parseInt(p[1], 10) - 1, parseInt(p[2], 10)));
  const sunday = new Date(dt);
  sunday.setUTCDate(dt.getUTCDate() + 6);
  return fmtDayNum(monday) + ' — ' + fmtDayNum(sunday.toISOString().slice(0, 10));
};
...
export default function WeeklyReport() {
  ...
  const load = async () => {
    ...
    try {
      ...
      // Группируем по неделе (ключ — понедельник). refund_items привязаны к item чека.
      const week = {}; // weekKey(пн) -> агрегаты
      const ensure = (wk) => {
        if (!week[wk]) week[wk] = {
          wk, salesSum: 0, retSum: 0, salesCost: 0, retCost: 0, salesQty: 0, retQty: 0,
        };
        return week[wk];
      };
      ...
      const list = Object.values(week).map(d => ({
        dk: d.wk,
        label: weekRange(d.wk),
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="sub">Продажи, возвраты и прибыль по каждой неделе за период</div>
        </div>
      </div>
