...
app.get('/api/thumb', async (req, res) => {
  try {
    ...
    const thumb = path.join(thumbDir, file.replace(/\.[a-z0-9]+$/i, '') + '.jpg');
    if (!fs.existsSync(thumb)) {
      await sharp(src).resize(120, 120, { fit: 'cover' }).jpeg({ quality: 80 }).toFile(thumb);
    }
    res.setHeader('Cache-Control', 'public, max-age=86400');
    res.sendFile(thumb);
  } catch (e) {
    // Если не удалось создать миниатюру — отдаём оригинал
    res.redirect('/uploads/' + path.basename(req.query.file || ''));
  }
});
