...
export default function AnnualReport() {
  ...
  const [rows, setRows] = useState([]);   // строки отчета
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Последний месяц в году, который уже прошел/идет (для будущих — данные еще не известны):
        const nowY = new Date().getFullYear();
        const nowM = new Date().getMonth(); // 0..11
        const lastMonth = year < nowY ? 11 : (year === nowY ? nowM : -1); // -1 => год еще не существует
        ...
        const M = Array.from({ length: 12 }, (_, idx) => ({
          idx,
          sales: 0,        // выручка по чекам (с учетом возвратов = total_amount, возвраты уже вычтены в total_amount чека)
          discounts: 0,
        ...
        // 1) Продажи и скидки по чекам года — сумма чеков уже финальная (total_amount), возвраты вычтены из нее.
        (recs || []).forEach(r => {
        ...
        // 5) Деньги на счетах на конец каждого месяца: баланс счета (уже учтен в данных?) В PnL:
        // деньги = сумма(баланс счета) + сумма движений транзакций.
        // Но так как транзакции хранят полную историю, а balance на счете обычно отражает текущее состояние,
        // чтобы получить «на конец месяца», добавляем движения до конца месяца к стартовому (баланс − все движения).
        // Надежнее: деньги на конец года считаем по балансам, а для промежуточных месяцев берем накопленно.
        // Для расчета «на конец месяца X» вычислим все движения до конца X и добавим стартовый капитал.
        // Стартовый = баланс всех счетов − (все движения по всем счетам за все время) [приближенно, если баланс хранит текущее]
        // В PnL используется: cash = sum(balance + sum(delta)). Это дает cash с учетом всех движений поверх баланса,
        // т.е. если транзакции не дублируют баланс, это скорее «полные деньги». Считаем накопленно от баланса:

        // Готовим накопительные движения до конца каждого месяца (income +, expense −, transfer учтем истинно по счету:
        ...
        // Для правильного остатка считаем приход/расход по каждому счету с учетом направленности (transfer идет
        // со счета и на счет — суммируем net по каждому счету).

        // «Деньги на конец месяца» — накопительный остаток по счетам (как в разделе «Счета»).
        // Остаток счета = balance (стартовый) + все движения на дату. На конец месяца X = Σ на конец X.
        // Деньги на конец месяца X = остаток на счетах на дату (последний день X).
        // Остаток счета = balance (стартовый при создании) + все движения по нему до этой даты
        ...
        // Стартовый остаток счета = balance. Пройдем месяцы, накапливая движения.
        const runningAcc = {};
        (accts || []).forEach(a => { runningAcc[a.id] = parseFloat(a.balance) || 0; });
        M.forEach((m) => {
          // Будущие месяцы (еще не наступили) — данных нет, показываем 0.
