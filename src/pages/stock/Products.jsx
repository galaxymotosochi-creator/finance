...
export default function Products() {
  ...
  const cellHtml = (col, p) => {
    switch(col) {
      case 'name': return `<div class="prod-name" style="cursor:pointer">${esc(p.name)}</div>`;
      case 'type':
        const typeLabel = p.type === 'service' ? 'Услуга' : p.type === 'combo' ? 'Комбо' : 'Товар';
        return `<span class="prod-cat">${typeLabel}</span>`;
      case 'category': return `<span class="prod-cat">${esc(CAT_LABELS[p.cat] || p.cat || '—')}</span>`;
      case 'cost': {
      ...
      case 'unit': return `<span style="color:#222">${esc(p.unit) || '—'}</span>`;
      case 'sku': return `<span style="color:#222">${esc(p.sku) || '—'}</span>`;
      case 'barcode': return `<span style="color:#222">${esc(p.barcode) || '—'}</span>`;
      case 'weight': {
        if (p.type==='service') return '<span style="color:#555">—</span>';
        const w = parseFloat(p.weight) || 0;
        return `<span>${w > 0 ? w + esc(p.weightUnit || p.weight_unit || 'кг') : '—'}</span>`;
      }
      ...
      case 'description': return `<span style="color:#222">${(p.description||p.desc) ? esc((p.description||p.desc)).substring(0,40)+((p.description||p.desc).length>40?'…':'') : '—'}</span>`;
      default: return '—';
    }
  };
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%',minHeight:0}}>
      ...
      <div className="sk-tablewrap">
        ...
        <div className="sk-fade sk-fade-r" style={{top:'50px',bottom:'auto',height:'30px',opacity:tblPos.right?1:0}} data-pos="bottom"></div>
            ...
            {filtered.length === 0 ? (
              ...
              <tr key={p.id} style={p.hidden ? {opacity:0.35,transition:'opacity .2s'} : {}}>
                {COL_ORDER.map(col => {
                  if (col === 'name' || activeCols.has(col)) {
                    if (col === 'name') {
                      return <td key={col} style={{cursor:'pointer',textAlign:'left',whiteSpace:'nowrap'}} onClick={() => setViewProduct(p)}>{p.photo_url ? <img src={thumbUrl(p.photo_url)} alt="" title="Открыть карточку" loading="lazy" decoding="async" style={{width:'55px',height:'55px',objectFit:'cover',borderRadius:'6px',verticalAlign:'middle',marginRight:'6px'}} /> : null}<div className="prod-name" style={{cursor:'pointer',display:'inline-block'}}>{p.name}{p.pending ? <span title="Ожидает синхронизации" style={{display:'inline-block',width:'12px',height:'12px',borderRadius:'50%',background:'#dc2626',boxShadow:'0 0 6px rgba(220,38,38,.6)',marginLeft:'6px',verticalAlign:'middle'}} /> : null}</div></td>;
                    }
