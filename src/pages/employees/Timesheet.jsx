      ...
      {showDay && (
        <div className="modal-overlay active" onClick={e => { if (e.target.className === 'modal-overlay active') setShowDay(null); }}>
          <div className="modal-box" style={{maxWidth:'520px'}}>
            ...
            <div className="sub">Отметки сотрудников и события</div>

            {/* Сотрудники */}
            <div className="emp-section-label">Сотрудники</div>
            {employees.length === 0 ? (
              <p style={{fontSize:'.82rem',color:'var(--muted)',marginBottom:'.5rem'}}>Нет сотрудников</p>
            ) : employees.map(emp => {
              const entry = getEntry(emp.id, showDay);
              const curStatus = entry ? entry.status : 'present';
              return (
                <div key={emp.id} style={{display:'flex',alignItems:'center',gap:'.5rem',marginBottom:'.3rem'}}>
                  <span style={{fontSize:'.82rem',minWidth:'120px',fontWeight:500}}>{emp.name}</span>
                  <select className="emp-rule-select" value={curStatus}
                    onChange={e => saveEntry(emp.id, 'status', e.target.value)} disabled={saving}
                    style={{fontSize:'.78rem',flex:1}}>
            ...
            {dayBonuses.length === 0 && <p style={{fontSize:'.82rem',color:'var(--muted)',marginBottom:'.35rem'}}>Нет бонусов</p>}
            {dayBonuses.map(e => (
            ...
            {editEmp && (
              <div className="emp-rule-row">
                <span style={{fontSize:'.8rem',minWidth:'120px'}}>{employees.find(em => em.id === editEmp)?.name}</span>
                <input type="number" className="emp-rule-input" placeholder="Сумма" id="tsBonusAmt" min="0" style={{width:'70px'}} />
                <input type="text" className="emp-rule-input" placeholder="За что" id="tsBonusComment" style={{width:'120px'}} />
                <button className="btn btn-primary" style={{fontSize:'.75rem',padding:'.25rem .5rem'}}
                  onClick={async () => {
                    const amt = parseFloat(document.getElementById('tsBonusAmt').value) || 0;
                    const cmt = document.getElementById('tsBonusComment').value;
                    if (!amt) return alert('Введите сумму');
                    const existing = getEntry(editEmp, showDay);
                    try {
                      if (existing) {
                        await supabase.from('timesheet_entries').update({
                          bonus_amount: (existing.bonus_amount || 0) + amt,
                          bonus_comment: existing.bonus_comment ? existing.bonus_comment + '; ' + cmt : cmt,
                        }).eq('id', existing.id);
                      } else {
                        await supabase.from('timesheet_entries').insert({
                          user_id: user.id, employee_id: editEmp, date: showDay,
                          status: 'present', bonus_amount: amt, bonus_comment: cmt,
                        });
                      }
                      await load(); setEditEmp(null);
                    } catch (err) { alert('Ошибка: ' + err.message); }
                  }}>➕</button>
              </div>
            )}
            <button className="emp-rule-add" onClick={() => setEditEmp(employees.length > 0 ? employees[0].id : null)}
              style={{marginBottom:'.5rem'}}>
              {editEmp ? '✕ Отмена' : '+ Добавить бонус'}
            </button>

            {/* Штрафы дня */}
            <div className="emp-section-label" style={{marginTop:'.5rem'}}>⛔ Штрафы</div>
            {dayDeducts.length === 0 && <p style={{fontSize:'.82rem',color:'var(--muted)',marginBottom:'.35rem'}}>Нет штрафов</p>}
            {dayDeducts.map(e => (
              <div key={e.id} className="emp-rule-line" style={{marginBottom:'.25rem'}}>
                <span style={{fontSize:'.82rem',fontWeight:500}}>{employees.find(em => em.id === e.employee_id)?.name || '—'}</span>
            ...
            {editEmp && (
              <div className="emp-rule-row" style={{marginTop:'.35rem'}}>
                <span style={{fontSize:'.8rem',minWidth:'120px'}}>{employees.find(em => em.id === editEmp)?.name}</span>
                <input type="number" className="emp-rule-input" placeholder="Сумма" id="tsDeductAmt" min="0" style={{width:'70px'}} />
                <input type="text" className="emp-rule-input" placeholder="За что" id="tsDeductComment" style={{width:'120px'}} />
                <button className="btn btn-primary" style={{fontSize:'.75rem',padding:'.25rem .5rem'}}
                  onClick={async () => {
                    const amt = parseFloat(document.getElementById('tsDeductAmt').value) || 0;
                    const cmt = document.getElementById('tsDeductComment').value;
                    if (!amt) return alert('Введите сумму');
                    const existing = getEntry(editEmp, showDay);
                    try {
                      if (existing) {
                        await supabase.from('timesheet_entries').update({
                          deduct_amount: (existing.deduct_amount || 0) + amt,
                          deduct_comment: existing.deduct_comment ? existing.deduct_comment + '; ' + cmt : cmt,
                        }).eq('id', existing.id);
                      } else {
                        await supabase.from('timesheet_entries').insert({
                          user_id: user.id, employee_id: editEmp, date: showDay,
                          status: 'present', deduct_amount: amt, deduct_comment: cmt,
                        });
                      }
                      await load(); setEditEmp(null);
                    } catch (err) { alert('Ошибка: ' + err.message); }
            ...
            {/* Итого дня */}
            {dayBonuses.length > 0 || dayDeducts.length > 0 ? (
              <div style={{
                marginTop:'.75rem',padding:'.5rem .65rem',background:'#f8f9fa',borderRadius:'.65rem',
                display:'flex',justifyContent:'space-between',fontSize:'.85rem'
              }}>
                <span>Бонусов: <b style={{color:'#16a34a'}}>{dayBonuses.reduce((s,e)=>s+Number(e.bonus_amount||0),0).toLocaleString()}₽</b></span>
                <span>Штрафов: <b style={{color:'#dc2626'}}>{dayDeducts.reduce((s,e)=>s+Number(e.deduct_amount||0),0).toLocaleString()}₽</b></span>
              </div>
            ) : null}
          </div>
        </div>
